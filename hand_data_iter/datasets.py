...
class LoadImagesAndLabels(Dataset):
                ...
                if len(hand_dict_) > 0:
                    for msg in hand_dict_:
                        ...
                        if vis:
                            ...
                            plot_box((x_min, y_min, x_max, y_max), img_, color=(255, 100, 100), label="hand",
                                     line_thickness=2)

                            offset_x = int((x_max - x_min) / 8)
                            offset_y = int((y_max - y_min) / 8)

                            pt_left = (x_min + random.randint(-offset_x, offset_x),
                                       (y_min + y_max) / 2 + random.randint(-offset_y, offset_y))
                            pt_right = (x_max + random.randint(-offset_x, offset_x),
                                        (y_min + y_max) / 2 + random.randint(-offset_y, offset_y))
                            ...
                            hand_rot, pts_tor_landmarks, _ = hand_alignment_aug_fun(img_ago, pt_left, pt_right,
                                                                                    ...
                                                                                    angle=angle_random,
                                                                                    desiredLeftEye=(scale_x, 0.5),
                                                                                    desiredFaceWidth=img_size[0],
    ...
    def __getitem__(self, index):
        ...
        if random.random() > 0.55:
            ...
            pt_left = (
            x_min + random.randint(-offset_x, offset_x), (y_min + y_max) / 2 + random.randint(-offset_y, offset_y))
            pt_right = (
            x_max + random.randint(-offset_x, offset_x), (y_min + y_max) / 2 + random.randint(-offset_y, offset_y))
            angle_random = random.randint(-180, 180)
            ...
            hand_rot, pts_tor_landmarks, _ = hand_alignment_aug_fun(img, pt_left, pt_right,
                                                                    ...
                                                                    desiredFaceWidth=self.img_size[0],
                                                                    desiredFaceHeight=None, draw_flag=False)
            if self.vis:
                pts_hand = {}
                for ptk in range(21):
                    xh, yh = pts_tor_landmarks[ptk][0], pts_tor_landmarks[ptk][1]
